...
async def expertise(request: Request):
    ...
    data = list()
    column_names = ['Номер записи', 'Магазин', 'Название акб', 'Номер', 'ФИО клиента', 'Когда обнаружили брак', 'Где документы', 'Где акб',
                    'Комментарий', 'Подменный АКБ', 'Кто принял(имя)', 'ссылка для отслеживания', 'Дата создания', 'Дата обновления']
